...
class Graphics:
    ...
    def __init__(self, player, enemies, map_, current_map, maps):
        ...
        Args:
            ...
            current_map (int): Map number

            maps (int): ammount of maps in general
        '''

        self.images_path = (Path(__file__).parent.parent).joinpath('img')
    ...
    def load_images(self):
        ...
        Args:
            path (string): path to map directory
        '''
        ...
        for file_ in files:
            self.images[file_.replace('.png', '')] = pygame.image.load(
                self.images_path.joinpath(file_))
    ...
    def __render_enemies_in_sight(self):
        ...
        # Get all enemies in distance of 12 block from the player
    ...
    def __render_enemy_bullets(self):
        ...
        for enemy in enemies:
            for bullet in enemy.weapon.bullets:
                # These parameters describe how bullet looks like
